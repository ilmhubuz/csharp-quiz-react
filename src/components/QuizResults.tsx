...
import type { AnswerReview, CollectionReviewResponse } from '../types/api';
...
export const QuizResults: React.FC<QuizResultsProps> = ({
    ...
    const [reviewData, setReviewData] =
        useState<CollectionReviewResponse | null>(null);
    ...
    useEffect(() => {
        const fetchResults = async () => {
            try {
                ...
                // Use the complete preview session endpoint to get immediate results
                const response = await resultService.completePreviewSession(
                    collectionId,
                    answerData
                );
                if (response.success && response.review) {
                    ...
                    const mappedResults: QuestionResult[] = questions.map(
                        (question, index) => {
                            const answerReview =
                                response.review.answerReviews.find(
                            ...
                            return {
                                ...
                                isCorrect: answerReview?.isCorrect || false,
                                questionNumber: index + 1,
                                correctAnswer: answerReview?.correctAnswer,
                                explanation: answerReview?.explanation,
                            };
                        }
                    );
        ...
        fetchResults();
    }, [questions, answers, collectionId]);
    ...
    const renderCorrectAnswer = (answer: string | string[] | undefined) => {
        ...
        // For code-based answers, render as code
        if (
            answer.includes('```') ||
            answer.includes('{') ||
            answer.includes(';')

...
export class QuestionService {
  ...
  async getQuestionsByCollection(
    ...
    try {
      const response = await apiClient.get<PaginatedApiResponse<QuestionResponse>>(
        `${this.baseEndpoint}?collectionId=${collectionId}&page=${page}&pageSize=${pageSize}`
      );
      return response;
    } catch (error) {
      console.error(`Failed to fetch questions for collection ${collectionId}:`, error);
      throw error;
    }
  }
  ...
  async getQuestionsByCollectionCode(
    ...
    try {
      ...
      console.error(`Failed to fetch questions for collection code ${collectionCode}:`, error);
      throw error;
    }
  }

  async getPreviewQuestions(collectionId: number): Promise<QuestionResponse[]> {
    try {
      ...
      console.error(`Failed to fetch preview questions for collection ${collectionId}:`, error);
      throw error;
    }
  }

  async getQuestionById(questionId: number): Promise<QuestionResponse | null> {
    try {
      const response = await apiClient.get<ApiResponse<QuestionResponse>>(
